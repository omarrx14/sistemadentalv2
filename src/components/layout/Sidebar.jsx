import { LayoutDashboard, Users, Calendar, Activity, DollarSign, Settings, LogOut, Search, X } from 'lucide-react';
...
const Sidebar = () => {
  const { activeView, setActiveView, patients, setSelectedPatient, selectedPatientId, isSidebarOpen, setSidebarOpen, clinicProfile } = useDentalStore();
  ...
  return (
    <>
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-dark/20 backdrop-blur-sm z-30 lg:hidden"
          onClick={() => setSidebarOpen(false)}
        />
      )}

      <div className={cn(
        "w-64 h-screen bg-white border-r border-slate-200 flex flex-col fixed left-0 top-0 z-40 transition-transform duration-300 lg:translate-x-0 overflow-y-auto",
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-8 px-2">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-xl text-dark">{clinicProfile.clinicName}</span>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
        ...
        <div className="mt-auto p-6 border-t border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-dark truncate">{clinicProfile.doctorName}</p>
              <p className="text-[10px] text-slate-500 truncate">{clinicProfile.specialty}</p>
            </div>
          </div>
